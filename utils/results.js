...
const RESULT_TEMPLATES = {
  ...
  outward_logic_order: {
    ...
    characters: [
      ...
      { name: '工藤新一', source: '《名侦探柯南》', reason: '看到线索→跑完推理→开口。脑子自动跑完了所有可能性。耍帅是副作用。' }
    ]
  },
  ...
  outward_feel_order: {
    ...
    dimensions: [
      ...
      { name: '你的判断系统', label: '准',
        analysis: '你判断一件事对不对，先想"这会让别人怎么想"，然后才是"这合理吗"。你有逻辑，只是你的逻辑系统里自动把人的感受放在了第一条。这让你的选择往往是最有人味的。代价是你偶尔要替别人收拾情绪的摊子。别人做了决定就走了，你留下来想把每个人的感受都安顿好。你最累的时候，往往不是因为事情多——你替别人承担了他们的情绪重量。' },
    ...
    characters: [
      { name: '灶门炭治郎', source: '《鬼灭之刃》', reason: '连敌人都会心疼。同理心自动运转，关不掉。最累的那个永远是他。' },
      { name: '莫妮卡·盖勒', source: '《老友记》', reason: '招待所有人、记住每个人的习惯、散场后一个人收拾。不安排好的话她不知道怎么放松。' }
    ]
  },
  ...
  outward_feel_flow: {
    ...
    characters: [
      { name: '漩涡鸣人', source: '《火影忍者》', reason: '他靠的不是嘴皮子。他真的相信每个人都有闪光点。冷掉的场子在他面前撑不过三分钟。' },
  ...
  inward_logic_order: {
    ...
    dimensions: [
      { name: '你跟世界的关系', label: '准',
        analysis: '你一个人待着的时候状态最好。对人有真实的兴趣，但你每跟人待完都需要一段独处来整理和归档。像电脑要关机重启，你要安静地把今天接收到的东西归位。你朋友不多，但你们之间的深度是别人几十个点头之交比不上的。你的精力有限，只够分配给真正值得的关系。直觉很准：谁是值得的，谁是浪费时间的，你第一次见面就知道了。你交得到朋友。你只是在等值得交的人。' },
  ...
  inward_logic_flow: {
    ...
    oneLiner: {
      ...
      mild: '你有一种"已经在别处想完了"的气质。别人觉得你随性，其实你的脑子一直没停。'
    },
    dimensions: [
      { name: '你的能量来源', label: '准',
        analysis: '你最好的想法产生于没人看的时候。散步、洗澡、睡前躺着的那十五分钟。人群里你话不多，但别人说的每一个字你都在听。你有一个特点：平时安静得像在观察，一旦有人聊到你真正在意的事，立刻从静音切到停不下来。这个反差经常把刚认识的人吓一跳。你喜欢的社交是"交换想法"，不是"交换近况"。一个晚上如果没有任何让你觉得"有意思"的对话，你会觉得浪费了时间。' },
  ...
  inward_feel_order: {
    ...
    dimensions: [
      ...
      { name: '你花了很多年学的功课', label: '有点像',
        analysis: '你学会了用稳定来保护自己。东西在它该在的位置、关系是稳定可依的、明天是可预期的。你发现外在的秩序能让内在的敏感安顿下来。东西不乱，心就不乱。这很聪明。但偶尔这套系统会过头：你会用"保持稳定"来躲开所有可能让你受伤的事。而有些最值得的东西——那个人、那个机会、那句话——需要你先放下这个保护罩。只对那个值得的人。你得自己判断谁值得。' }
    ],
  ...
  inward_feel_flow: {
    ...
    characters: [
      ...
      { name: '碇真嗣', source: '《EVA》', reason: '接收器太灵敏了——一个眼神、一句话的语气——他全收到了。在一个音量太大的世界里，只能把自己调成静音。' }
    ]
  }
}
...
function calculateResult(answers, options) {
  ...
  if (scoredAnswerCount === 0) {
    ...
    const customShortDimensions = [
      ...
      {
        name: '更适合你的读法',
        zinger: '你这次最有信息量的地方，是你拒绝的那些选项——比你选的说了更多。',
    ...
    const customContradictionHook = {
      ...
      partialReveal: '这背后是你对表达准确度有要求。真正值得看的，是你反复想修正的那些地方……',
      unlockPercent: 24
    }
    return {
      personalization,
      ...
      dimensions: [
        {
          name: '你的答案方式',
          label: '待确认',
          analysis: '这份结果没有足够的选项数据来给你贴类型。你选择自己写，说明预设选项没有接住你当下的真实反应。这个动作本身有信息量：你更在意准确表达，而不是随便选一个接近的答案。'
        },
        {
          name: '更适合你的读法',
          label: '待确认',
  ...
  return {
    ...
    characters: pickCharacters(mappedType, {
      intensity: intensity,
      sessionIndex: options ? options.sessionIndex : 0
    }),
    // 命门句 + 短维度 + 矛盾钩子
...
const { pickCharacters } = require('./character-pool')
